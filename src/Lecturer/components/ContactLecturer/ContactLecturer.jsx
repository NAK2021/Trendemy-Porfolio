...
import styles from "./ContactLecturer.module.css"; // Import CSS module for styling
...
export default function ContactLecturer() {
   ...
   */
  const { ref: BackgroundContactLecturerRef, inView: BackgroundContactLecturerInView } =
    useInView({
   ...
   */
  const { ref: ContactLecturerTitleRef, inView: ContactLecturerTitleInView } =
    useInView({
      triggerOnce: true, // Animation should trigger only once
    });
  ...
  return (
    ...
    <>
      <section className={styles.contactLecturer} id="contact-advice">
        <div
          ref={BackgroundContactLecturerRef} // Reference for background image container
          className={`${styles.backgroundContactLecturer} animate__animated ${
            BackgroundContactLecturerInView ? "animate__fadeInUp" : "" // Apply fadeInUp animation if in view
          }`}
        >
          <img
            src={ContactCourseImage} // Contact image
            alt="Liên hệ"
            className={styles.imageContactLecturer}
          />
        </div>

        <div
          ref={ContactLecturerTitleRef} // Reference for title container
          className={`${styles.contactLecturerTitle} animate__animated ${
            ContactLecturerTitleInView ? "animate__fadeInUp" : "" // Apply fadeInUp animation if in view
          }`}
        >
          <h1>LIÊN HỆ TƯ VẤN</h1>
          <h4>
            Hãy để{" "}
            <span className={styles.trendemyContactLecturer}>TRENDEMY</span> giúp
            bạn giải đáp mọi thắc mắc!
          </h4>

          <div className={styles.contactLecturerForm}>
            <form onSubmit={handleSubmit}>
              {/* Input for Name */}
              <div className={styles.formGroupLecturer}>
                <input
                  type="text"
                  id="name"
                  name="name"
                  ...
                  required
                />
                <i className={`fas fa-user ${styles.iconContactLecturer}`}></i>
              </div>
              ...
              <div className={styles.formGroupLecturer}>
                <input
                  ...
                  required
                />
                <i className={`fas fa-phone ${styles.iconContactLecturer}`}></i>
              </div>
              ...
              <div className={styles.formGroupLecturer}>
                <input
                  ...
                  required
                />
                <i className={`fas fa-envelope ${styles.iconContactLecturer}`}></i>
              </div>
              ...
              {/* Submit button */}
              <div className={styles.btnLecturer}>
                <button
                  className={styles.buttonRegisterContactLecturer}
                  type="submit"
                >
                  Liên hệ tư vấn
                </button>
