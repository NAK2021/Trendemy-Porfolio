...
/**
 * Course component to display individual course details with animation.
...
function Course({ img, avatar, instructor, rating, description, lesson, student, level }) {
  // Use useInView hook to trigger animation on scroll for individual courses
  const { ref: courseRef, inView: courseInView } = useInView({ triggerOnce: true });

  return (
    <div
      ref={courseRef}
      className={`animate__animated ${courseInView ? 'animate__fadeInUp' : ''} ${styles.containerCourses}`}
    >
      {/* Course image */}
  ...
  */
  ...
  return (
    <section className={styles.coursesSectionPlatform} id="typical-courses">
      ...
      <div
        ref={ContainerCoursesSectionRef}
