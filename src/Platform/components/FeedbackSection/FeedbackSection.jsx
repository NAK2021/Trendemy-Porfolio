...
export default function FeedbackSection() {
    return (
        <section className={styles.feedbackSectionPlatform} id="feedback">
            <div className={styles.feedbackSectionPlatformContainer}>
                {/* Section title */}
                <img src={iconBlueCircle} className={styles.iconBlueCircle} />
                <h1 className={styles.feedbackSectionPlatformTitleFirst}>Cảm nghĩ</h1>
                <h1 className={styles.feedbackSectionPlatformTitleSecond}>Học viên của chúng tôi đã nói gì</h1>
                <img src={iconRedCircle} className={styles.iconRedCircle} />
                <div className={styles.feedbackSectionPlatformContainerFeedback}>
                    <div className={styles.feedbackSectionContainerImg}>
                        {/* Feedback section image */}
                        <img className={styles.feedbackSectionPlatformImg} src={feedbackSectionImg} alt="feedbackSectionImg"/>
                    </div>
                    ...
                    <Swiper
                        ...
                        modules={[Pagination]}
                        loop={true}
                        className={styles.feedbackSectionPlatfromContainerSwiper}
                    >
                        {feedbacks.map((feedback) => (
                            <SwiperSlide key={feedback.key}>
                                <div className={styles.feedbackSectionPlatformCard}>
                                    {/* Feedback content */}
                                    <p className={styles.feedbackSectionPlatformContent}>{feedback.content}</p>
                                    {/* Quote icon */}
                                    <PhQuotesThin className={styles.feedbackSectionPlatformVector} />
                                    <div className={styles.feedbackSectionPlatformContainerInfoUser}>
                                        {/* User image */}
                                        <img className={styles.feedbackSectionPlatformImgUser} src={feedback.image} alt={feedback.user} />
                                        <div className={styles.feedbackSectionPlatformContainerInfoUser}> 
                                            {/* User name and position */}
                                            <h5 className={styles.feedbackSectionPlatformNameUser}>{feedback.user}</h5>
                                            <h6 className={styles.feedbackSectionPlatformPositionUser}>{feedback.position}</h6>
                                        </div>
                                    </div>
                                </div>
                            </SwiperSlide>
